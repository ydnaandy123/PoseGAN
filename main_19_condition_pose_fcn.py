...
"""
import numpy as np
from model_conditional_fcn import DCGAN_conditional_fcn
from utils import pp, show_all_variables, config_check
import tensorflow as tf

flags = tf.app.flags
flags.DEFINE_string("dataset_name", "condition_pose_fcn", "The name of dataset [celebA, mnist, lsun]")
...
def main(_):
    ...
    with tf.Session(config=run_config) as sess:
        dcgan_conditional_fcn = DCGAN_conditional_fcn(sess, config, needCondition=True)
    show_all_variables()

    if FLAGS.mode == 'train':
        dcgan_conditional_fcn.train(config)
    elif FLAGS.mode == 'test':
        dcgan_conditional_fcn.test(config)
    else:
        if not dcgan_conditional_fcn.load(FLAGS.checkpoint_dir)[0]:
