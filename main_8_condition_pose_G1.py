...
"""
import numpy as np
from model_conditional_G1 import DCGAN_conditional_G1
from utils import pp, show_all_variables, config_check
import tensorflow as tf

flags = tf.app.flags
flags.DEFINE_string("dataset_name", "condition_pose_G1", "The name of dataset [celebA, mnist, lsun]")
...
flags.DEFINE_float("L1_lambda", 100.0, "L1_lambda [100.0]")
...
def main(_):
    ...
    with tf.Session(config=run_config) as sess:
        dcgan_conditional_g1 = DCGAN_conditional_G1(sess, config, needCondition=True)
    show_all_variables()

    if FLAGS.mode == 'train':
        dcgan_conditional_g1.train(config)
    elif FLAGS.mode == 'test':
        dcgan_conditional_g1.test(config)
    else:
        if not dcgan_conditional_g1.load(FLAGS.checkpoint_dir)[0]:
