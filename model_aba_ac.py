...
class ABAACCity(Model):
    def __init__(self, sess, config, gf_dim=64, df_dim=64):
        # init
        super(ABAACCity, self).__init__(sess=sess, config=config)
        # Hyper parameter
    ...
    def get_sample(self):
        ...
        sample_images = get_batch_images_norm(image_files, self.need_flip, scale=0.5)
        sample_conditions = get_batch_images_norm(conditions_files, self.need_flip, scale=0.5)
        # Sample visual
    ...
    def get_batch(self, files):
        ...
        images = get_batch_images_norm(image_files, self.need_flip, scale=0.25)
        conditions = get_batch_images_norm(conditions_files, self.need_flip, scale=0.25)
