...
class InpaintingCity(Model):
    def __init__(self, sess, config, gf_dim=64, df_dim=64):
        # init
        self.mask_dir = config.mask_dir
        super(InpaintingCity, self).__init__(sess=sess, config=config)
        # Random mask
        self.mask = sorted(glob(os.path.join(self.mask_dir, self.input_fname_pattern)))
        # Hyper parameter
    ...
    def get_valid_data(self):
        ...
        data_condition = sorted(glob(os.path.join(self.mask_dir, self.input_fname_pattern)))
    ...
    def get_sample(self):
        ...
        sample_conditions = get_batch_images(conditions_files, self.need_flip, scale=1.0)
