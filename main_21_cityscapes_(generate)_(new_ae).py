import numpy as np
from model_new_ae import GenerateNewAECity
from utils import pp, show_all_variables, config_check_directory
import tensorflow as tf

flags = tf.app.flags
# Experiments setting
flags.DEFINE_string("name", "cityscapes_(generate)_(new_ae)", "The name of dataset [celebA, mnist, lsun]")
flags.DEFINE_string("mode", "generate", "network mode [train, test]")
...
def main(_):
    ...
    with tf.Session(config=run_config) as sess:
        model = GenerateNewAECity(sess, FLAGS)
        model.build_model()
    show_all_variables()
    # launch mode
    if FLAGS.mode == 'train':
