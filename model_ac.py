...
class ACCity(Model):
    def __init__(self, sess, config, gf_dim=64, df_dim=64):
        ...
        super(ACCity, self).__init__(sess=sess, config=config)
        # Hyper parameter
    ...
    def generator(self, condition, training=True, reuse=False):
        with tf.variable_scope("generator") as scope:
            ...
            classify = deconv2d(elu(d7), [-1, self.s_h, self.s_w, self.num_of_class], name='g_class')
            # d8 is (256 x 256 x output_c_dim)
            return tf.nn.tanh(d8), classify
