...
class DCGAN_conditional_fcn(object):
    def __init__(self, sess, config, z_dim=100,  gf_dim=64, df_dim=64, gfc_dim=1024, dfc_dim=1024, needCondition=False):
        """
        Args:
    ...
    def inference(self, image, keep_prob):
        ...
        with tf.variable_scope("inference"):
            ...
            w8 = weight_variable([1, 1, 4096, self.image_dim], name="W8")
            b8 = bias_variable([self.image_dim], name="b8")
            conv8 = conv2d_basic(relu_dropout7, w8, b8)
            # annotation_pred1 = tf.argmax(conv8, dimension=3, name="prediction1")

            # now to upscale to actual image size
            deconv_shape1 = image_net["pool4"].get_shape()
            w_t1 = weight_variable([4, 4, deconv_shape1[3].value, self.image_dim], name="W_t1")
            ...
            deconv_shape3 = tf.stack([shape[0], shape[1], shape[2], self.image_dim])
            w_t3 = weight_variable([16, 16, self.image_dim, deconv_shape2[3].value], name="W_t3")
            b_t3 = bias_variable([self.image_dim], name="b_t3")
    ...
    def generator(self, z, condition):
        with tf.variable_scope("generator") as scope:
            if self.needCondition:
                ...
                mean = model_data['normalization'][0][0][0]
                mean_pixel = np.mean(mean, axis=(0, 1))
                # TODO: does this change make sense?
                processed_image = condition - mean_pixel
