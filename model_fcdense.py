...
class FCDENSE(object):
    ...
    def inference(self, image):
        ...
        with tf.variable_scope("inference"):
            ...
            # TODO: no relu? fuse? concat? batch_size
        ...
        with tf.variable_scope("generator"):
            if self.need_condition:
                ...
                for i in range(n_pool):
                    ...
                    stack = transition_up(skip_connection_list[i], block_to_upsample, n_filters_keep, training=training, name=name)
                    # Dense Block
                    block_to_upsample = []
                ...
                output_layer = conv2d(stack, self.num_of_class, d_h=1, d_w=1, name='outputlayer')
                return output_layer
            else:
    ...
    def test_classify(self):
        ...
        if could_load:
            ...
            #if not os.path.exists(config.test_dir):
            #    os.makedirs(config.test_dir)
            val_dir = '/data/vllab1/Github/pose_gan/dataset/CITYSCAPES_DATASET/train/semantic_id'
            val = glob(os.path.join(val_dir, self.input_fname_pattern))
            batch_idxs = len(val) // self.sample_num
            # every mini-batch
            for idx in range(0, batch_idxs):
                # Get feeds
                batch_files = self.data[idx * self.sample_num:(idx + 1) * self.sample_num]
                print('{:d}/{:d}'.format(idx, batch_idxs))
                batch_images, batch_conditions = get_image_condition_classify(
                    [batch_files], '/data/vllab1/Github/pose_gan/dataset/CITYSCAPES_DATASET/train/image')
