...
class GenerateNewAECity(Model):
    def __init__(self, sess, config):
        # init
        super(GenerateNewAECity, self).__init__(sess=sess, config=config)
        # Network architect
    ...
    def get_sample(self):
        # Get sample
        images, conditions = get_city_generate_image(self.sample_files, self.condition_dir_val, self.need_flip)
    ...
    def get_batch(self, files):
        images, conditions = get_city_generate_image(files, self.condition_dir)
    ...
    def generator(self, condition, training=True, reuse=False):
        with tf.variable_scope("generator") as scope:
            ...
            d1 = bn(deconv2d(elu(e8), [-1, deconv_shape1[1], deconv_shape1[2], self.gf_dim * 8],
                             name='g_d1'), training=training)
            d1 = tf.concat([d1, e7], 3)
            # d1 is (2 x 2 x self.gf_dim*8*2)
            deconv_shape2 = e6.get_shape().as_list()
            d2 = bn(deconv2d(elu(d1), [-1, deconv_shape2[1], deconv_shape2[2], self.gf_dim * 8],
                             name='g_d2'), training=training)
            d2 = tf.concat([d2, e6], 3)
            # d2 is (4 x 4 x self.gf_dim*8*2)
            deconv_shape3 = e5.get_shape().as_list()
            d3 = bn(deconv2d(elu(d2), [-1, deconv_shape3[1], deconv_shape3[2], self.gf_dim * 8],
                             name='g_d3'), training=training)
            d3 = tf.concat([d3, e5], 3)
            # d3 is (8 x 8 x self.gf_dim*8*2)
            deconv_shape4 = e4.get_shape().as_list()
            d4 = bn(deconv2d(elu(d3), [-1, deconv_shape4[1], deconv_shape4[2], self.gf_dim * 8],
                             name='g_d4'), training=training)
            d4 = tf.concat([d4, e4], 3)
            # d4 is (16 x 16 x self.gf_dim*8*2)
            deconv_shape5 = e3.get_shape().as_list()
            d5 = bn(deconv2d(elu(d4), [-1, deconv_shape5[1], deconv_shape5[2], self.gf_dim * 4],
                             name='g_d5'), training=training)
            d5 = tf.concat([d5, e3], 3)
            # d5 is (32 x 32 x self.gf_dim*4*2)
            deconv_shape6 = e2.get_shape().as_list()
            d6 = bn(deconv2d(elu(d5), [-1, deconv_shape6[1], deconv_shape6[2], self.gf_dim * 2],
                             name='g_d6'), training=training)
            d6 = tf.concat([d6, e2], 3)
            # d6 is (64 x 64 x self.gf_dim*2*2)
            deconv_shape7 = e1.get_shape().as_list()
            d7 = bn(deconv2d(elu(d6), [-1, deconv_shape7[1], deconv_shape7[2], self.gf_dim],
                             name='g_d7'), training=training)
            d7 = tf.concat([d7, e1], 3)
            # d7 is (128 x 128 x self.gf_dim*1*2)
            deconv_shape8 = condition.get_shape().as_list()
            d8 = deconv2d(elu(d7), [-1, deconv_shape8[1], deconv_shape8[2], self.image_dim], name='g_d8')
    ...
    def pred(self):
        ...
        if could_load:
            ...
            val_conditions = glob(os.path.join(self.condition_dir_val, self.input_fname_pattern))
            ...
            for idx in range(0, len(val_conditions)):
                ...
                batch_image = [scipy.misc.imresize(
                    scipy.misc.imread(batch_files).astype(np.uint8)[:, :, :3], 1.0).astype(np.float32) / 127.5 - 1.0]
    ...
    def generate(self):
        ...
        if could_load:
            ...
            for idx in range(0, len(self.data_val)):
                ...
                name = batch_files.split('/')[-1]
                print('{:d}/{:d}: {}'.format(idx, len(self.data_val), name))
                batch_image = [scipy.misc.imresize(
                    scipy.misc.imread(batch_files).astype(np.uint8)[:, :, :3], 1.0).astype(np.float32) / 127.5 - 1.0]
                sample_feed = {self.is_training: False, self.keep_prob: 1.0, self.conditions: batch_image}
                # Feed
                samples_g = self.sess.run(self.fake_images, feed_dict=sample_feed)
                # Save
                scipy.misc.imsave('./{}/{}'.format(result_dir, name), merge(denorm_image(samples_g), [1, 1]).astype(np.uint8))
                #label_v = label_id_visual_single(samples_g_out)
                #scipy.misc.imsave('./{}/{}'.format(visual_dir, name), label_v.astype(np.uint8))
                # break
        else:
